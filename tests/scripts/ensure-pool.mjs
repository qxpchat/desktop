...
//
// Reads `tests/pool.json`; for each slot 1..POOL_SIZE either:
...
// ---- pool file ----
//
// `tests/pool.json` shape:
//   { "relayQr": "...", "poolSize": 10, "displayNamePrefix": "qxp e2e",
//     "accounts": [ { "email": "...", "password": "..." }, ... ] }
// `accounts[n-1]` is slot n (1-based). Gitignored — holds real creds.

async function readPool(p) {
  try {
    const parsed = JSON.parse(await readFile(p, 'utf8'));
    return {
      relayQr: parsed.relayQr,
      poolSize: parsed.poolSize,
      displayNamePrefix: parsed.displayNamePrefix,
...
async function writePool(p, pool) {
  const tmp = `${p}.tmp`;
 ...
 *    trios:  7-8-9
 *    free:   10 */
async function ensureTrios(accounts) {
  const slots = [];
  for (let n = 1; n <= POOL_SIZE; n++) {
    const a = accounts[n - 1];
    if (!a?.email || !a?.password) continue;
    slots.push({ slot: n, email: a.email, password: a.password });
  }
...
async function ensureTemplates(accounts) {
...
async function main() {
  const pool = await readPool(POOL_PATH);
  ...
  try {
    for (let n = 1; n <= POOL_SIZE; n++) {
      const existing = pool.accounts[n - 1];
      ...
      try {
        ...
        pool.accounts[n - 1] = { email: newEmail, password: newPassword };
    ...
    // Persist the effective config so pool.json documents what the pool
    // was created against.
    pool.relayQr ??= RELAY_QR;
    pool.poolSize ??= POOL_SIZE;
    pool.displayNamePrefix ??= DISPLAY_PREFIX;

    await writePool(POOL_PATH, pool);
  } finally {
    rpc.close();
    daemon.proc.kill();
    await rm(accountsDir, { recursive: true, force: true });
  ...
  const tplResult = await ensureTemplates(pool.accounts);
  console.log(
  ...
  const trioResult = await ensureTrios(pool.accounts);
  console.log(
